...
def test(num_episodes, agent, env, evaluate, model_path, visualize=True, debug=False):
    ...
    for i in range(num_episodes):
        validate_reward, validate_sharpe, validate_sortino, validate_calmar, validate_mdd, validate_cum_returns = evaluate(
            env, policy, debug=debug, visualize=visualize, save=False
        )
        if debug:
            prYellow("[Evaluate] #{}: mean_reward:{} mean_sharpe:{} mean_sortino:{} mean_calmar:{} mean_mdd:{} mean_cum_returns:{}".format(
                i, validate_reward, validate_sharpe, validate_sortino, validate_calmar, validate_mdd, validate_cum_returns
            ))
...
if __name__ == "__main__":
    ...
    with open("./data/traindata_ussnp500_rl.pkl", "rb") as f:
        ...
        for i in range(len(data_train)):
            for key in data_train[i].keys():
                if isinstance(data_train[i][key], list):
                    # Only convert flat lists to numpy arrays
                    if not any(isinstance(x, (list, tuple)) for x in data_train[i][key]):
                        try:
                            data_train[i][key] = np.array(data_train[i][key])
                        except ValueError:
                            # If conversion fails, keep as list
                            continue

    with open("./data/testdata_ussnp5000_rl.pkl", "rb") as f:
        data_test = pickle.load(f)
        # Convert data to numpy arrays if needed
        for i in range(len(data_test)):
            for key in data_test[i].keys():
                if isinstance(data_test[i][key], list):
                    # Only convert flat lists to numpy arrays
                    if not any(isinstance(x, (list, tuple)) for x in data_test[i][key]):
                        try:
                            data_test[i][key] = np.array(data_test[i][key])
                        except ValueError:
                            # If conversion fails, keep as list
                            continue
